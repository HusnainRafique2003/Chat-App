  ...
  <div class="flex h-full flex-col gap-4">
    <section class="rounded-[2rem] border border-[var(--ui-border)] bg-[var(--ui-bg)] p-6 shadow-[var(--shadow-md)]">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p class="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--ui-text-dimmed)]">
            Message workspace
          </p>
          <h2 class="mt-2 text-2xl font-black text-[var(--ui-text-highlighted)]">
        ...
        <div class="flex w-full max-w-xl flex-col gap-3 sm:flex-row">
          <UInput
            v-model="channelId"
            placeholder="Enter channel ID (ObjectId)"
            class="w-full"
          />
          <BaseButton
            label="Load Messages"
    ...
    <section
      v-if="channelId.trim()"
      class="flex-1 overflow-hidden rounded-[2rem] border border-[var(--ui-border)] bg-[var(--ui-bg)] shadow-[var(--shadow-md)]"
    >
      <MessageList
    ...
    <section
      v-else
      class="flex flex-1 items-center justify-center rounded-[2rem] border border-[var(--ui-border)] bg-[var(--ui-bg)] shadow-[var(--shadow-md)]"
    >
      <div class="text-center">
        <UIcon
          name="i-mdi-chat-outline"
          class="w-16 h-16 text-[var(--ui-text-dimmed)] mx-auto mb-4"
        />
        <p class="text-[var(--ui-text-muted)]">
          Enter a channel ID to load messages.
        </p>
      </div>
    </section>
  </div>
