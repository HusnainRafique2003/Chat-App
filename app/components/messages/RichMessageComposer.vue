...
const allEmojis = computed(() => {
  const emojis = emojiCategories[selectedCategory.value as keyof typeof emojiCategories] || emojiCategories.smileys
...
<template>
  <Teleport to="body">
    ...
    <Transition name="pop">
      <div
        v-if="showLinkDialog"
        class="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100vw-1.5rem)] max-w-[360px] rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-bg)] shadow-2xl p-4 sm:p-5"
        @click.stop
      >
        ...
        <div class="space-y-3">
          <div>
            ...
            <input
              ...
              class="w-full rounded-lg border border-[var(--ui-border)] bg-[var(--ui-bg-elevated)] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--ui-primary)]/30"
            >
          </div>
          <div>
    ...
    <Transition name="pop">
      <div
        ...
        class="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100vw-1.5rem)] max-w-[340px] rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-bg)] shadow-2xl p-4 sm:p-5"
        @click.stop
      >
        ...
        >
          <div class="relative h-16 w-16 rounded-full bg-red-500/10 flex items-center justify-center">
            ...
            <UIcon
              name="i-lucide-mic"
              class="h-7 w-7 text-red-500 relative z-10"
            />
          </div>
          ...
          <p class="text-xl font-mono font-bold tabular-nums text-red-500">
            {{ recordingTime }}
          </p>
    ...
    <Transition name="pop">
      ...
      >
        ...
        <Transition name="fade">
          <div
            v-if="scheduleLabel"
            class="mt-3 rounded-lg bg-[var(--ui-primary)]/5 border border-[var(--ui-primary)]/15 px-3 py-2 flex items-center gap-2"
          >
            <UIcon
              name="i-lucide-clock"
              class="h-4 w-4 text-[var(--ui-primary)] shrink-0"
            />
            <p class="text-xs font-bold text-[var(--ui-primary)]">
              Sends on {{ scheduleLabel }}
            </p>
          </div>
        </Transition>
        <p
    ...
    <Transition name="pop">
      ...
      >
        <div
          ref="emojiPickerRef"
          class="w-[95vw] max-w-2xl bg-[var(--ui-bg)] border border-[var(--ui-border)] shadow-2xl rounded-3xl overflow-auto max-h-[85vh] relative z-[9999]"
          @click.stop
        >
          <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-[var(--ui-border)] sticky top-0 bg-[var(--ui-bg)] z-10">
            <h3 class="text-xl font-bold tracking-tight text-[var(--ui-text)]">
              Select Emoji 🎉
            </h3>
            <UButton
              icon="i-lucide-x"
          ...
          <div class="px-5 py-3 border-b border-[var(--ui-border)] sticky top-[57px] bg-[var(--ui-bg)] z-10">
            <input
              ...
              class="w-full rounded-lg border border-[var(--ui-border)] bg-[var(--ui-bg-elevated)] px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-[var(--ui-primary)]/30"
            >
          </div>
          <div class="flex gap-1 px-4 py-2 border-b border-[var(--ui-border)] overflow-x-auto sticky top-[107px] bg-[var(--ui-bg)] z-10">
            <button
              ...
              :class="selectedCategory === cat.id ? 'bg-[var(--ui-primary)] text-white' : 'hover:bg-[var(--ui-bg-elevated)] text-[var(--ui-text-dimmed)]'"
          ...
          <div class="p-5 max-h-[400px] overflow-y-auto">
            <div
              v-if="allEmojis.length === 0"
              class="text-center py-8 text-[var(--ui-text-dimmed)]"
            >
              No emojis found
            </div>
            <div
            ...
            >
              <button
                ...
                :key="idx"
                class="h-10 w-10 rounded-lg hover:bg-[var(--ui-bg-elevated)] transition-all flex items-center justify-center text-2xl cursor-pointer hover:scale-110"
    ...
    <Transition name="pop">
      <div
        ...
        class="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100vw-1.5rem)] max-w-[340px] rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-bg)] shadow-2xl p-4"
        @click.stop
      >
        ...
        <div
          v-else-if="mentionUsers.length > 0"
  ...
  <div
    v-bind="$attrs"
    class="relative z-20 border-t border-[var(--ui-border)] bg-[var(--ui-bg)]"
  >
    <div class="w-full">
      <div class="relative flex flex-col border border-[var(--ui-border)] border-t-0 bg-[var(--ui-bg-elevated)] shadow-lg focus-within:ring-2 focus-within:ring-[var(--ui-primary)]/20 transition-all overflow-hidden">
        ...
        <div class="flex items-center gap-1 px-2 py-1.5 sm:px-3 border-b border-[var(--ui-border)] bg-gradient-to-b from-[var(--ui-bg)] to-[var(--ui-bg-muted)]/20">
          ...
          <AppTooltip text="Send message (Enter)">
            <UButton
              ...
              color="primary"
              size="sm"
              class="h-9 px-4 shrink-0 cursor-pointer rounded-lg shadow-md shadow-[var(--ui-primary)]/20 font-semibold transition-all duration-200 hover:shadow-lg hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              :loading="loading"
        ...
        <div class="relative flex min-h-[60px] max-h-[160px] flex-col overflow-y-auto sm:max-h-[140px]">
          <editor-content
            :editor="editor"
            class="flex-1 px-3 py-2 text-sm focus:outline-none sm:px-4 sm:py-2.5 prose prose-sm max-w-none dark:prose-invert leading-relaxed custom-editor"
          ...
          <Transition name="slide-up">
            <div
              v-if="selectedFile"
              class="mx-4 my-3 p-3 rounded-2xl flex items-center gap-4 bg-gradient-to-r from-[var(--ui-bg-elevated)] to-[var(--ui-bg)]/50 border border-[var(--ui-border)] shadow-xl backdrop-blur-sm transition-all duration-300 hover:shadow-2xl ring-1 ring-[var(--ui-primary)]/10 hover:ring-[var(--ui-primary)]/20"
              :class="{ 'ring-green-500/30 from-green-500/5 to-green-500/10 border-green-500/30': fileStatus === 'valid' && selectedFile.type.startsWith('audio/'), 'ring-red-500/30 from-red-500/5 to-red-500/10 border-red-500/30': fileStatus === 'invalid', 'ring-[var(--ui-primary)]/20 from-[var(--ui-primary)]/5 to-[var(--ui-primary)]/10 border-[var(--ui-primary)]/20': fileStatus === 'idle' }"
            >
