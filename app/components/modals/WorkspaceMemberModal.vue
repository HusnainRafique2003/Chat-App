    ...
    <div class="flex flex-col gap-4">
      <div v-if="isCurrentUserCreator" class="shrink-0">
        ...
        <div class="relative mt-2">
          <UInput v-model="searchQuery" type="search" placeholder="Search available users by name or email..." >
            <template #leading><UIcon name="i-lucide-search" class="h-4 w-4" /></template>
          </UInput>
          <div v-if="isLoadingUsers" class="absolute right-3 top-1/2 -translate-y-1/2">
      ...
      <div class="flex flex-col gap-6">
        <div v-if="isCurrentUserCreator" class="flex flex-col gap-3">
          <div>
            <h3 class="text-sm font-semibold text-[var(--ui-text)] mb-3">Available to Add ({{ filteredAvailableUsers.length }})</h3>
            <div class="space-y-2 max-h-[28vh] overflow-y-auto pr-2 pb-1 [scrollbar-width:thin]">
              
              <div v-if="isLoadingUsers && filteredAvailableUsers.length === 0" class="text-sm text-[var(--ui-text-muted)] text-center py-4">
                Loading available users...
              </div>

              <div v-for="(member, index) in filteredAvailableUsers" :key="member.id" class="flex items-center justify-between p-3 rounded-lg bg-[var(--ui-bg-muted)] border border-[var(--ui-border)] hover:bg-[var(--ui-bg-muted)]/80 transition-colors">
                ...
                <button type="button" @click="handleAddMember(member.id)" :disabled="addingMemberId !== null" class="shrink-0 px-3 py-1.5 rounded-lg bg-[var(--ui-primary)] text-white text-xs font-medium hover:bg-[var(--ui-primary)]/90 transition-colors disabled:opacity-50">
                  <span v-if="addingMemberId !== member.id">Add</span>
                  <UIcon v-else name="i-lucide-loader" class="h-3 w-3 animate-spin" />
                </button>
              </div>

              <p v-if="!isLoadingUsers && filteredAvailableUsers.length === 0" class="text-sm text-[var(--ui-text-muted)] text-center py-4">
        ...
        <div class="flex flex-col gap-3" :class="{ 'border-t border-[var(--ui-border)] pt-4': isCurrentUserCreator }">
          <h3 class="text-sm font-semibold text-[var(--ui-text)] shrink-0">Workspace Members ({{ currentMembers.length }})</h3>
          <div class="space-y-2 overflow-y-auto pr-2 pb-1 [scrollbar-width:thin]" :class="isCurrentUserCreator ? 'max-h-[35vh]' : 'max-h-[50vh]'">
            <div v-for="(member, index) in currentMembers" :key="member.id || member.user_id" class="flex items-center justify-between p-3 rounded-lg bg-[var(--ui-bg-muted)] border border-[var(--ui-border)]">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    <p class="text-sm font-medium text-[var(--ui-text)] truncate">{{ member.name || 'Unknown User' }}</p>
                    <span v-if="isCreator(member.id || member.user_id)" class="shrink-0 rounded-full bg-[var(--ui-primary)]/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-[var(--ui-primary)]">Creator</span>
                  </div>
                  <p class="text-xs text-[var(--ui-text-muted)] truncate">{{ member.email }}</p>
                </div>
              </div>
              <button v-if="isCurrentUserCreator && !isCreator(member.id || member.user_id)" type="button" @click="handleRemoveMember(member.id || member.user_id)" :disabled="removingMemberId !== null" class="shrink-0 px-3 py-1.5 rounded-lg bg-red-500/10 text-red-500 text-xs font-medium hover:bg-red-500/20 transition-colors disabled:opacity-50">
